...
public class MainGame extends JPanel implements Runnable {
    ...
    public static Image[] tileset_tree = new Image[10];

    public static int myWidth, myHeight;
    public static int coinage = 50, health = 30;
    ...
    public int[] mobNumber = {3, 5, 6, 13, 16, 17, 19, 23, 24, 25,28, 31, 34, 37, 38, 40, 43, 47, 48, 50, 55};
    public int[] ninja = {1,4,10,14};
    ...
    public void define() {// define: định nghĩa
        ...
        tileset_mob[1] = new ImageIcon("./src/Game_Image/Enemy_Image/boss.png").getImage();
        tileset_mob[2] = new ImageIcon("./src/Game_Image/Enemy_Image/ninja.png").getImage();
        ...
        if (spawnFrame >= spawnTime) {
            for (int i = 0; i < mobs.length; i++) {
                if (!mobs[i].inGame) {
                    ...
                    else if(ninja[k] == mobSpawned){
                        mobs[ninja[k]].towerDamage = 1;
                        mobs[ninja[k]].spawnMob(Value.ninja);
                        if(k<2) k++;
                    }
                    else{
