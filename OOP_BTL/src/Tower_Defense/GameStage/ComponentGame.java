...
public class ComponentGame extends JPanel {
                    ...
                    for (int y = 0; y < MainGame.room.block.length; y++) {
                        for (int x = 0; x < MainGame.room.block[0].length; x++) {
                            if (MainGame.room.block[y][x].contains(MainGame.mse)) {
                                if (MainGame.room.block[y][x].groundId != Value.groundRoad &&
                                        MainGame.room.block[y][x].groundId != Value.groundRoad1 &&
                                        MainGame.room.block[y][x].groundId != Value.groundRoad2 &&
                                        MainGame.room.block[y][x].groundId != Value.groundRoad3 &&
    ...
    public void draw(Graphics g) {
        for (int i = 0; i < button.length; i++) {
            ...
            if (buttonID[i] != Value.airAir) {
                g.drawImage(MainGame.tileset_air[buttonID[i]], button[i].x + itemIn, button[i].y + itemIn,
